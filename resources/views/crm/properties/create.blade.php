...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <!-- Header Block -->
    <div>
        <h1 class="text-3xl font-serif font-extrabold text-white">Add Property Listing</h1>
        <p class="text-sm text-slate-400 mt-1">Create a new premium real estate listing to be showcased on the consumer website.</p>
    </div>

    <!-- Form Card -->
    <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 shadow-md">
        <form action="{{ route('crm.properties.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <input id="title" name="title" type="text" required value="{{ old('title') }}" placeholder="e.g. 3BHK Premium Sea View Apartment"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="price" name="price" type="number" required value="{{ old('price') }}" placeholder="e.g. 15000000"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
                    </div>

                    <div class="space-y-1.5">
                        <label for="avg_price_per_sqft" class="text-xs font-bold text-slate-400">Avg. Price / sq.ft</label>
                        <input id="avg_price_per_sqft" name="avg_price_per_sqft" type="number" step="0.01" value="{{ old('avg_price_per_sqft') }}" placeholder="e.g. 10230"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
                    </div>

                    <div class="space-y-1.5">
                        <label for="possession_date" class="text-xs font-bold text-slate-400">Possession Date</label>
                        <input id="possession_date" name="possession_date" type="date" value="{{ old('possession_date') }}"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="configuration" name="configuration" required class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">
                            <option value="1BHK" {{ old('configuration') === '1BHK' ? 'selected' : '' }}>1 BHK Flat</option>
                            <option value="2BHK" {{ old('configuration') === '2BHK' ? 'selected' : '' }}>2 BHK Flat</option>
                            <option value="3BHK" {{ old('configuration') === '3BHK' ? 'selected' : '' }}>3 BHK Flat</option>
                            <option value="4BHK" {{ old('configuration') === '4BHK' ? 'selected' : '' }}>4 BHK Flat</option>
                            <option value="Villa" {{ old('configuration') === 'Villa' ? 'selected' : '' }}>Villa / House</option>
                            <option value="Plot" {{ old('configuration') === 'Plot' ? 'selected' : '' }}>Plot / Land</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="bedrooms" name="bedrooms" type="number" value="{{ old('bedrooms') }}" placeholder="e.g. 3"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="bathrooms" name="bathrooms" type="number" value="{{ old('bathrooms') }}" placeholder="e.g. 3"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="area" name="area" type="number" value="{{ old('area') }}" placeholder="e.g. 1800"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="developer_name" name="developer_name" type="text" value="{{ old('developer_name') }}" placeholder="e.g. Hiranandani Developers"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="partner_id" name="partner_id" class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">
                            ...
                            @foreach (($partners ?? collect()) as $partner)
                                <option value="{{ $partner->id }}" {{ (string) old('partner_id') === (string) $partner->id ? 'selected' : '' }}>{{ $partner->company_name }} ({{ ucfirst($partner->type) }})</option>
                            @endforeach
                        </select>
                    </div>
                            ...
                            @foreach (($partners ?? collect()) as $partner)
                                <label class="flex items-center gap-2 rounded-lg border border-slate-850 p-3 text-xs text-slate-200 cursor-pointer hover:bg-slate-850/20 transition-colors">
                                    <input type="checkbox" name="partners[]" value="{{ $partner->id }}" 
                                        ...
                                        {{ is_array(old('partners')) && in_array($partner->id, old('partners')) ? 'checked' : '' }}>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="contact_whatsapp" name="contact_whatsapp" type="text" value="{{ old('contact_whatsapp') }}" placeholder="+91 9559992958"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="rera_number" name="rera_number" type="text" value="{{ old('rera_number') }}" placeholder="e.g. P51800001234"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="state" name="state" type="text" value="{{ old('state') ?? 'Maharashtra' }}" placeholder="e.g. Maharashtra"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="status" name="status" required class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">
                            <option value="publish" {{ old('status') === 'publish' ? 'selected' : '' }}>Published (Visible on site)</option>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <input id="latitude" name="latitude" type="number" step="0.00000001" value="{{ old('latitude') }}" placeholder="e.g. 19.117"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input id="longitude" name="longitude" type="number" step="0.00000001" value="{{ old('longitude') }}" placeholder="e.g. 72.906"
                                class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-3 py-3 outline-none text-sm text-slate-250">
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5 md:col-span-2 grid md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input id="featured_image_url" name="featured_image_url" type="text" value="{{ old('featured_image_url') }}" placeholder="https://example.com/image.jpg"
                    ...
                    <div class="space-y-1.5 md:col-span-2 grid md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <textarea id="gallery_urls" name="gallery_urls" rows="2" placeholder="https://example.com/gallery1.jpg&#10;https://example.com/gallery2.jpg"
                                class="w-full mt-1 bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-2.5 outline-none text-sm text-slate-250 placeholder-slate-650">{{ old('gallery_urls') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <textarea id="address" name="address" rows="3" placeholder="e.g. Hiranandani Gardens, Powai, Mumbai, Maharashtra 400076"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">{{ old('address') }}</textarea>
                    </div>

                    <!-- Description -->
                    <div class="space-y-1.5 md:col-span-2">
                        <label for="description" class="text-xs font-bold text-slate-400">Property Description</label>
                        <div id="quill-editor" class="w-full">{!! old('description') !!}</div>
                    ...
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <textarea id="video_urls" name="video_urls" rows="3" placeholder="One YouTube or MP4 URL per line"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250">{{ old('video_urls') }}</textarea>
                    </div>
                </div>
            </div>

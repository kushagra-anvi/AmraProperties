...
<div class="max-w-4xl mx-auto space-y-6">
    <!-- Back Link -->
    <a href="{{ route('crm.sales.show', $salesPerson->id) }}" class="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-white transition-colors">
        <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i> Back to Representative Profile
    </a>

    <!-- Header Block -->
    <div>
        <h1 class="text-3xl font-serif font-extrabold text-white">Edit Sales Representative</h1>
        <p class="text-sm text-slate-400 mt-1">Modify account details, update local service coverage areas, or manage status parameters.</p>
    </div>

    <!-- Form Card -->
    <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 shadow-md">
        <form action="{{ route('crm.sales.update', $salesPerson->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="name" name="name" type="text" required value="{{ old('name') ?? $salesPerson->name }}" placeholder="e.g. Rahul Sharma"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="email" name="email" type="email" required value="{{ old('email') ?? ($salesPerson->user ? $salesPerson->user->email : '') }}" placeholder="rep@amra.com"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
                    </div>

                    <!-- Password (Optional) -->
                    <div class="space-y-1.5">
                        <label for="password" class="text-xs font-bold text-slate-400">Reset Password (Optional)</label>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="phone" name="phone" type="text" required value="{{ old('phone') ?? $salesPerson->phone }}" placeholder="e.g. +91 99999 11111"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="location" name="location" type="text" required value="{{ old('location') ?? $salesPerson->location }}" placeholder="e.g. Mumbai Office"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
                    </div>

                    <!-- Account Status -->
                    <div class="space-y-1.5">
                        <label for="is_active" class="text-xs font-bold text-slate-400">Account Status <span class="text-rose-500">*</span></label>
                        <select id="is_active" name="is_active" required 
                            ...
                            <option value="1" {{ (old('is_active') ?? $salesPerson->is_active) ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ !(old('is_active') ?? $salesPerson->is_active) ? 'selected' : '' }}>Inactive / Suspended</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-1.5 mt-4">
                    ...
                    <input id="service_areas_input" name="service_areas_raw" type="text" 
                        value="{{ old('service_areas_raw') ?? (is_array($salesPerson->service_areas) ? implode(', ', $salesPerson->service_areas) : '') }}" 
                        placeholder="e.g. Powai, Hiranandani, Hazratganj"
                        class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
            ...
            <div class="pt-6 border-t border-slate-800 flex justify-end gap-3">
                <a href="{{ route('crm.sales.show', $salesPerson->id) }}" class="bg-slate-800 hover:bg-slate-755 text-slate-350 font-bold text-xs px-6 py-3.5 rounded-xl transition-all">
                    Cancel
                </a>

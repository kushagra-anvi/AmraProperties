...
@section('title', 'Edit Buyer Lead')

@section('content')
<div class="max-w-4xl mx-auto space-y-6">
    <!-- Back Link -->
    <a href="{{ route('crm.b2c.show', $lead->id) }}" class="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-white transition-colors">
        <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i> Back to Buyer Details
    </a>

    <!-- Header Block -->
    <div>
        <h1 class="text-3xl font-serif font-extrabold text-white">Modify Buyer Profile</h1>
        <p class="text-sm text-slate-400 mt-1">Update property preferences and contact details for {{ $lead->name }}</p>
    </div>

    <!-- Form Card -->
    <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 shadow-md">
        <form action="{{ route('crm.b2c.update', $lead->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div class="grid gap-6 md:grid-cols-3">
                    ...
                    <div class="md:col-span-1 space-y-1.5">
                        ...
                        <input id="name" name="name" type="text" required value="{{ old('name') ?? $lead->name }}" placeholder="John Doe"
                    ...
                    <div class="md:col-span-1 space-y-1.5">
                        ...
                        <input id="phone" name="phone" type="text" required value="{{ old('phone') ?? $lead->phone }}" placeholder="e.g. +91 9999988888"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
                    </div>
                    ...
                    <div class="md:col-span-1 space-y-1.5">
                        ...
                        <input id="email" name="email" type="email" value="{{ old('email') ?? $lead->email }}" placeholder="buyer@example.com"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
                    </div>
                </div>
            </div>
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="property_type" name="property_type" required 
                            ...
                            <option value="flat" {{ (old('property_type') ?? $lead->property_type) === 'flat' ? 'selected' : '' }}>Flat / Apartment</option>
                            <option value="plot" {{ (old('property_type') ?? $lead->property_type) === 'plot' ? 'selected' : '' }}>Plot / Land</option>
                            <option value="villa" {{ (old('property_type') ?? $lead->property_type) === 'villa' ? 'selected' : '' }}>Villa / Independent House</option>
                            <option value="commercial" {{ (old('property_type') ?? $lead->property_type) === 'commercial' ? 'selected' : '' }}>Commercial Office</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="configuration" name="configuration" required 
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-200">
                            <option value="1BHK" {{ (old('configuration') ?? $lead->configuration) === '1BHK' ? 'selected' : '' }}>1 BHK</option>
                            <option value="2BHK" {{ (old('configuration') ?? $lead->configuration) === '2BHK' ? 'selected' : '' }}>2 BHK</option>
                            <option value="3BHK" {{ (old('configuration') ?? $lead->configuration) === '3BHK' ? 'selected' : '' }}>3 BHK</option>
                            <option value="4BHK" {{ (old('configuration') ?? $lead->configuration) === '4BHK' ? 'selected' : '' }}>4 BHK</option>
                            <option value="Plot" {{ (old('configuration') ?? $lead->configuration) === 'Plot' ? 'selected' : '' }}>Plot</option>
                            <option value="Studio" {{ (old('configuration') ?? $lead->configuration) === 'Studio' ? 'selected' : '' }}>Studio</option>
                        </select>
                    </div>

                    <!-- Budget Min -->
                    <div class="space-y-1.5">
                        <label for="budget_min" class="text-xs font-bold text-slate-400">Min Budget (INR)</label>
                        <input id="budget_min" name="budget_min" type="number" min="0" value="{{ old('budget_min') ?? $lead->budget_min }}" placeholder="e.g. 5000000"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
                    </div>

                    <!-- Budget Max -->
                    <div class="space-y-1.5">
                        <label for="budget_max" class="text-xs font-bold text-slate-400">Max Budget (INR)</label>
                        <input id="budget_max" name="budget_max" type="number" min="0" value="{{ old('budget_max') ?? $lead->budget_max }}" placeholder="e.g. 20000000"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
                    </div>
                </div>
            </div>

            <!-- Section 3: Locations & Address -->
            <div class="space-y-4 pt-4">
                <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-800 pb-2">Target Locations</h3>
                ...
                <div class="grid gap-6 md:grid-cols-3">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="pincode" name="pincode" type="text" value="{{ old('pincode') ?? $lead->pincode }}" placeholder="e.g. 400076"
                            class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="assigned_sales_person_id" name="assigned_sales_person_id" 
                            ...
                            @foreach ($salesPeople as $person)
                                <option value="{{ $person->id }}" {{ (old('assigned_sales_person_id') ?? $lead->assigned_sales_person_id) == $person->id ? 'selected' : '' }}>
                                    {{ $person->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-1.5 mt-4">
                    ...
                    <input id="preferred_locations_input" name="preferred_locations_raw" type="text" value="{{ old('preferred_locations_raw') ?? (is_array($lead->preferred_locations) ? implode(', ', $lead->preferred_locations) : '') }}" placeholder="e.g. Powai Lake, Hiranandani Gardens, Hazratganj Metro"
                        class="w-full bg-slate-950 border border-slate-800 focus:border-amra-primary rounded-xl px-4 py-3 outline-none text-sm text-slate-250 placeholder-slate-650">
            ...
            <div class="pt-6 border-t border-slate-800 flex justify-end gap-3">
                <a href="{{ route('crm.b2c.show', $lead->id) }}" class="bg-slate-800 hover:bg-slate-755 text-slate-350 font-bold text-xs px-6 py-3.5 rounded-xl transition-all">
                    Cancel
                </a>
